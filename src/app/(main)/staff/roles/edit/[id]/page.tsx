...
export default function EditRolePage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setErrors({
        submit: "Failed to update role. Please try again.",
      });
    } finally {
      setLoading(false);
    }
  ...
  if (initialLoad) {
    return (
      <div className="h-full bg-white flex items-center justify-center">
        <div className="w-8 h-8 border-4 border-[#0F75BD] border-t-transparent rounded-full animate-spin" />
      </div>
    );
  }
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="flex items-center justify-between pt-4">
            <button
              ...
              disabled={loading}
              className="px-6 py-2.5 border border-red-500 text-red-600 rounded-xl hover:bg-red-50 transition-colors font-medium disabled:opacity-50 disabled:cursor-not-allowed flex items-center gap-2"
            >
